...
public:
    ...
    void onOrientationData(myo::Myo* myo, uint64_t timestamp, const myo::Quaternion<float>& quat)
    {
        ...
        prevPitch = pitch_w;
        pitch_w = static_cast<int>((pitch + (float)M_PI/2.0f)/M_PI * 180);
        prevYaw=yaw_w;
        yaw_w = static_cast<int>((yaw + (float)M_PI)/(M_PI * 2.0f) * 180);
    }
    ...
    //yawToRealYaw converts between raw input yaw to our real, normalized yaw, and also deals w/ wraparound
    //then it takes the realYaw and converts it into a keystroke
    int yawToKeyRealYaw(int yaw, int prevYaw)
    {
        int realYaw=yaw;
        int wrapVal=170;
        int dif=prevYaw-yaw;
        //check for warparound
        if (std::abs(dif)>10000){
            if (dif<0){ //going west
                realYaw=wrapVal+yaw;
            }else{ //going east
                realYaw=yaw-wrapVal-1;
            }
        }
        return realYaw;
    }
    
    bool yawToKeyPress(int yaw,int prevYaw){
        int realYaw=yawToKeyRealYaw(yaw, prevYaw);
        ...
        //so that the sound doesn't repeat twice unnecesarily
        if (std::abs(countFirstDiff-countFirst) > 3 and lagged) {
            lagged = false;
        }
        
        return (isIn == true); //returns true if we just did a DJ in movement
    }
    ...
    void print()
    {
        ...
        if (onArm) {
            ...
            std::cout << "pose " << poseString << " | ";
            std::cout << "callibrateBeginning " << callibrateBeginning << " | ";
            std::cout << "callibrateEnd " << callibrateEnd << std::endl;
            
            //setup instructions
            if (not callibrated and not callibrating) {
                std::cout << "Callibrate the yaw (begin by making a fist at the chest position and swinging arm further away from body, maintaining a tight fist the whole time)";
                if (poseString == "fist") {
                    callibrating = true;
                }
            }
            
            if (callibrating and poseString == "fist") {
                std::cout << "Maintain fist, callibrating..." << std::endl;
                
                countFirst++;
                
                if (countFirst == 1) {
                    callibrateBeginning = yaw_w;
                }
                
                callibrateEnd = yaw_w;
            }
            
            if (callibrating and poseString != "fist") { //callibration has ended
                std::cout << "Callibration ended" << std::endl;
                prevYaw = yaw_w;
                callibrated = true;
                callibrating = false;
            }
